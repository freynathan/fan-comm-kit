export function ForBrandsSection() {
  const stats = [
    { value: "$200B", desc: "spent on social ads annually with near-zero AI visibility" },
    { value: "48hrs", desc: "until every social post disappears forever" },
    { value: "0", desc: "other platforms where brand presence is permanent and AI-readable" },
    { value: "€149/mo", desc: "for permanent verified presence across the entire passion network" },
  ];

  return (
    <section className="w-full py-20 px-6">
      <div className="max-w-5xl mx-auto text-center">
        <p className="text-[11px] font-semibold tracking-[0.15em] uppercase text-[hsl(var(--grey-text))] mb-3">
          For brands
        </p>
        <h2 className="text-[36px] font-bold text-[hsl(var(--navy))] leading-tight mb-2">
          Your audience is already here.
        </h2>
        <p className="text-[16px] text-[hsl(var(--grey-text))] mb-12 max-w-2xl mx-auto">
          Permanent, community-verified, AI-readable brand presence — for less than the cost of one sponsored post.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left mb-10">
          {/* Left — brand page mock */}
          <div className="rounded-xl border border-[hsl(var(--grey-border))] p-6 bg-white">
            <div className="flex items-center gap-2 mb-4">
              <div className="flex-1 flex items-center h-10 rounded-lg border border-[hsl(var(--grey-border))] px-3">
                <span className="text-[13px] text-[hsl(var(--grey-text))]">yourbrand.com</span>
              </div>
              <button
                className="h-10 px-4 rounded-lg text-[13px] font-semibold text-white whitespace-nowrap"
                style={{ backgroundColor: "#0C447C" }}
              >
                Generate →
              </button>
            </div>
            <div className="rounded-lg border border-[hsl(var(--grey-border))] p-4 bg-[hsl(var(--muted))]">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-full bg-[hsl(var(--grey-border))]" />
                <div>
                  <p className="text-[13px] font-semibold text-[hsl(var(--navy))]">tobe.fan/of/YourBrand</p>
                  <p className="text-[11px] text-[hsl(var(--grey-text))]">Verified brand presence</p>
                </div>
              </div>
              <div className="space-y-1.5">
                <div className="w-full h-2.5 rounded bg-[hsl(var(--grey-border))]" />
                <div className="w-3/4 h-2.5 rounded bg-[hsl(var(--grey-border))]" />
                <div className="w-1/2 h-2.5 rounded bg-[hsl(var(--grey-border))]" />
              </div>
            </div>
            <p className="text-[12px] text-[hsl(var(--grey-text))] mt-3 text-center">
              Paste your URL. Your community page is live in seconds.
            </p>
          </div>

          {/* Right — stats */}
          <div className="flex flex-col justify-center gap-4">
            {stats.map((s) => (
              <div key={s.value} className="flex items-start gap-3 pl-3" style={{ borderLeft: "3px solid #0D9488" }}>
                <span className="text-[22px] font-bold text-[hsl(var(--navy))] whitespace-nowrap">{s.value}</span>
                <span className="text-[13px] text-[hsl(var(--grey-text))] pt-1">{s.desc}</span>
              </div>
            ))}
          </div>
        </div>

        <a
          href="/brands"
          className="inline-flex h-11 px-8 items-center rounded-lg text-[14px] font-semibold border-2 transition-colors hover:bg-[hsl(var(--muted))]"
          style={{ borderColor: "#0C447C", color: "#0C447C" }}
        >
          See your brand's presence →
        </a>
        <p className="text-[12px] text-[hsl(var(--grey-text))] mt-3">
          First 10 founding brand partners get lifetime preferred pricing
        </p>
      </div>
    </section>
  );
}
